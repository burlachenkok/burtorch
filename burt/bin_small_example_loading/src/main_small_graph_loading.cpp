...
int load_benchmark(int argc, char** argv, const char* test_name)
{
	...
	{
		bool loaded = loadFromFile<true/*load_vaues*/, false/*load_grads*/>({a,b,c,d,e,f,g}, "my_file_f.bin");
		assert(loaded);
	}
...
int load_benchmark_light(int argc, char** argv, const char* test_name)
{
	...
	for (size_t i = 0; i < kIterations; ++i)
	{
		...
		bool loaded = loadComputeGraphContextFromFile<true/*load_vaues*/, false/*load_grads*/, decltype(g)>(fname.str().c_str());
		assert(loaded);
