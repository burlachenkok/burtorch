...
# -----------------------------------------------------------------------------
# HACK TO ENSURE ONLY 1 THREAD FOR BACKPROP
torch.set_num_threads(1)             # Limits the number of threads for CPU computations
torch.set_num_interop_threads(1)     # Limits the number of threads for inter-op parallelism
os.environ["OMP_NUM_THREADS"] = "1"  # Limit OpenMP threads
os.environ["MKL_NUM_THREADS"] = "1"  # Limit MKL threads
os.environ["NUMEXPR_NUM_THREADS"] = "1"  # Limit NumExpr threads (if applicable)
p = psutil.Process(os.getpid())

#p.cpu_affinity([1])  # CPU indices start from 0

# psutil has issues with implementation of cpu_affinity in macos
# https://github.com/giampaolo/psutil/issues/1826
...
def create_datasets(input_file):
    ...
    test_set_size = min(0, int(len(words) * 0.0)) # 10% of the training set, or up to 1000 examples
...
class InfiniteDataLoader:
    ...
    def __init__(self, dataset, **kwargs):
        train_sampler = torch.utils.data.RandomSampler(dataset, replacement=not True, num_samples=int(1e10))
...
if __name__ == '__main__':
    ...
    print(f"model params dtype: {next(model.parameters()).dtype}")
    ...
    print("train set size in samples: ", len(train_dataset))
    ...
    time_to_process_avg = 0.0
    time_to_process_sqr_avg = 0.0
    ...
    while True:
        ...
        time_to_process_avg = ((e - 1)/(e) * time_to_process_avg + time_to_process/(e))
        time_to_process_sqr_avg = ((e - 1) /(e) * time_to_process_sqr_avg + time_to_process * time_to_process /(e))


        if step % 10 == 0:
            print(f"step {step} | loss {loss.item():.4f} | avg time {time_to_process_avg*1000:.2f}ms | std.dev for time {((time_to_process_sqr_avg - time_to_process_avg * time_to_process_avg)**0.5)*1000:.2f}ms ")
            print(f"model #params: {sum(p.numel() for p in model.parameters())}")
        ...
        # evaluate the model
        if False and step > 0 and step % 500 == 0:
